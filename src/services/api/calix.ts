...
function buildAuthPayload() {
  return {
    client_id: process.env.EXPO_PUBLIC_CALIX_CLIENT_ID,
    client_secret: process.env.EXPO_PUBLIC_CALIX_CLIENT_SECRET,
    username: process.env.EXPO_PUBLIC_CALIX_USERNAME,
    password: process.env.EXPO_PUBLIC_CALIX_PASSWORD,
    tenant: process.env.EXPO_PUBLIC_CALIX_TENANT, // Optional
    grant_type: 'password',
  };
}
...
export async function getAccessToken(): Promise<string> {
  ...
  const authUrl = process.env.EXPO_PUBLIC_CALIX_AUTH_URL;
  if (!authUrl) {
    throw new Error('EXPO_PUBLIC_CALIX_AUTH_URL is not configured');
  }
  ...
  try {
    ...
    if (tokenCache.refreshToken) {
      try {
        ...
        refreshParams.append('client_secret', process.env.EXPO_PUBLIC_CALIX_CLIENT_SECRET || '');

        const refreshResponse = await axios.post(authUrl, refreshParams.toString(), {
          headers: {
            'Content-Type': 'application/x-www-form-urlencoded',
            'X-Calix-ClientID': process.env.EXPO_PUBLIC_CALIX_CLIENT_ID || '',
    ...
    if (!payload.client_id || !payload.username || !payload.password) {
      throw new Error('Missing required auth credentials (EXPO_PUBLIC_CALIX_CLIENT_ID, EXPO_PUBLIC_CALIX_USERNAME, EXPO_PUBLIC_CALIX_PASSWORD)');
    }
...
export async function callCalixServiceInsights<T>(
  ...
  const baseUrl = process.env.EXPO_PUBLIC_CALIX_BASE_URL;
  if (!baseUrl) {
    throw new Error('EXPO_PUBLIC_CALIX_BASE_URL is not configured');
  }

  const token = await getAccessToken();
  const clientId = process.env.EXPO_PUBLIC_CALIX_CLIENT_ID;
